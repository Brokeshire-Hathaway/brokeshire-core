...
def get_system_prompt(unique_entities: dict[int, dict[str, Any]]):
    return f"""You are a natural language processing expert that links a named entity to a unique entity. Select one of the unique entities that matches the named entity. If there isn't a clear match, show low confidence. You must generate a response following the JSON schema below.
...
async def llm_entity_match(
    ...
    messages: list[ChatCompletionMessageParam] = [
        ...
        {"role": "user", "content": named_entity},
    ]
...
async def link_entity(
    ...
    llm_matches = await llm_entity_match(named_entity, fuzzy_entity_matches, llm_keys)
