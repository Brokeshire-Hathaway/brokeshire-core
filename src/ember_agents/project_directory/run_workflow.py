...
async def run_workflow_saved(daily_update):
  ...
  with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_headers_response.json", "r") as f:
      data = f.read()
...
def run_conversion(date):
    ...
    all_queries = ""
    with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_news_items_{date}.json", "r") as f:
      data = f.read()
      news_queries = main_news_items(data)
      all_queries += "\n".join(news_queries)
    print(f"=====CURRENTLY running conversion; news_queries: {news_queries}")
    with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_project_updates_{date}.json", "r") as f:
      data = f.read()
      project_updates_queries = main_project_updates(data)
      print("@@@@@@@@@@@@@@@@")
      print(data)
      all_queries += "\n".join(project_updates_queries)
    print(f"=====CURRENTLY running conversion; project_updates_queries: {project_updates_queries}")
    with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_threads_and_reads_items_{date}.json", "r") as f:
      data = f.read()
      threads_and_reads_queries = main_threads_and_reads(data)
      all_queries += "\n".join(threads_and_reads_queries)
    print(f"=====CURRENTLY running conversion; threads_and_reads_queries: {threads_and_reads_queries}")
    with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_launches_items_{date}.json", "r") as f:
      data = f.read()
    ...
    with open(f"src/ember_agents/project_directory/docs/cached_daily_updates/parsed_new_projects_items_{date}.json", "r") as f:
      data = f.read()
      new_project_queries = main_new_projects(data)
      all_queries += "\n".join(new_project_queries)
    print(f"=====CURRENTLY running conversion; new_project_queries: {new_project_queries}")
    with open(f"src/ember_agents/project_directory/docs/populate_database/cypher_scripts/all_queries_{date}.txt", "w") as f:
      f.write(all_queries)
